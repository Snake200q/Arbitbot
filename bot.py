            ...
            for m in data.get("data", []):
                try:
                    ...
                    for t in tokens:
                        ...
                        if out == "YES": yes_p = price
                        elif out == "NO": no_p = price
                    if not yes_p or not no_p: continue
                    if yes_p < 0.01 or no_p < 0.01: continue
                    total = yes_p + no_p
        ...
        while True:
            ...
            for m in markets:
                try:
                    ...
                    if not out_prices or not outcomes: continue
                    if isinstance(out_prices, str):
                        out_prices = json.loads(out_prices)
                    if isinstance(outcomes, str):
                        outcomes = json.loads(outcomes)
                    if len(out_prices) != 2: continue
        ...
        for _ in range(3):
            ...
            for m in data:
                try:
                    ...
                    if prob <= 0.02 or prob >= 0.98: continue
                    yes_p = prob
                    no_p = 1 - prob
                    eff = yes_p + no_p - 0.02
        ...
        for m in data.get("markets", []):
            try:
                for c in m.get("contracts", []):
                    ...
                    if yb < 0.02 or nb < 0.02: continue
                    total = yb + nb
                    spread = round((1 - total) * 100, 2)
                    cname = c.get("name", "")
                    question = m.get("name", "")[:120] if cname.lower() in ["yes","no",""] else f"{m.get('name','')} — {cname}"[:120]
                    results.append({
                        "platform": "PredictIt",
                        "question": question,
                        "yes": round(yb*100, 1),
...
def get_limitless():
    results = []
    try:
        h = {**HEADERS, "Origin": "https://limitless.exchange", "Referer": "https://limitless.exchange/"}
        data = safe_get("https://limitless.exchange/api/v1/markets", headers=h, timeout=20)
        if not data: return results
...
def get_opinion():
    ...
    for url, params in endpoints:
        h = {**HEADERS, "Origin": "https://opinion-market.com", "Referer": "https://opinion-market.com/"}
        data = safe_get(url, params=params if params else None, headers=h, timeout=15)
...
def get_predictfun():
    ...
    for url in endpoints:
        h = {**HEADERS, "Origin": "https://predict.fun", "Referer": "https://predict.fun/"}
        data = safe_get(url, headers=h, timeout=15)
        if not data: continue
        markets = data if isinstance(data, list) else data.get("markets", data.get("data", []))
        for m in markets:
            try:
            ...
            for j in range(i+1, len(markets)):
                ...
                if diff >= MIN_SPREAD:
                    cross.append({
                        ...
                        "diff": round(diff, 1), "volume": max(a.get("volume",0), b.get("volume",0)), "exact": True
                    })
    return cross
...
async def do_scan(bot):
    ...
    if total == 0:
        await send_safe(bot, f"😴 *Арбитражей >{MIN_SPREAD}% не найдено*\n🌐 Рынков: {len(all_markets)}\n🕐 {now}",
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if d == "scan":
            ...
            await query.edit_message_text(f"✅ Мин. спред: *{MIN_SPREAD}%*", parse_mode="Markdown", reply_markup=main_menu())
        elif d.startswith("lim_"):
            MAX_RESULTS = int(d.split("_")[1])
            await query.edit_message_text(f"✅ Макс. результатов: *{MAX_RESULTS}*", parse_mode="Markdown", reply_markup=main_menu())
    except Exception as e:
